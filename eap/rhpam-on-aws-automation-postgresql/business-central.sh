...
#  scp -i ${SSH_PEM_FILE} ./resources/jboss-eap/* ${SSH_USER_ID}@$BUSINESS_CENTRAL_IP:/tmp
#  scp -i ${SSH_PEM_FILE} ./resources/rhpam/* ${SSH_USER_ID}@$BUSINESS_CENTRAL_IP:/tmp
  ...
  scp -i ${SSH_PEM_FILE} ./resources/jboss-eap/eap-auto-updated.xml $SSH_USER_ID@$BUSINESS_CENTRAL_IP:/tmp/eap-auto.xml
...
# TODO: move in separate runtime properties file
...
#extract rhsso-adapter files into temp directory;
  execute "cd /tmp; sudo unzip ${EAP_SSO_ADAPTER} -d ${EAP_HOME}"
  ...
  execute "cd /tmp; sudo java -jar ${RHPAM_INSTALLER} /tmp/bc-auto.xml"
  # TODO Use jboss-cli instead
